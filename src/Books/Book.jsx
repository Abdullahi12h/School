...
function DisplayBooks() {
  ...
  const fetchBooks = async () => {
    try {
      const res = await axios.get("https://backendschool1.onrender.com/read/books");
      setBooks(res.data);
    } catch (err) {
      console.error(err);
    }
  ...
  const handleDelete = async (id) => {
    try {
      await axios.delete(`https://backendschool1.onrender.com/book/delete/${id}`);
      fetchBooks();
    } catch (err) {
      console.error(err);
    }
        ...
        <div className="flex-1 p-10 bg-white min-h-screen">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
            {books.map((book) => (
              ...
              >
                <img
                  src={`https://backendschool1.onrender.com/allimages/${book.img}`}
                  alt={book.img}
